...
class FileIO6
{
	public void writeToFile(String s1, String s2, String s3, String s4)
	{
		...
		try
		{
			FileWriter myWriter = new FileWriter("/home/guddu/Lumiq/Ergo/Ergo_Sep2_sample_output.csv",true);
...
public class ErgoMatcherV2
{	
	/*Main function*/
	public static void main(String[] args)
	throws Exception
	{
		double coeff_id=0,coeff_address2,coeff_email,coeff_name=0,coeff_contact, coeff_dob, coeff_gender,numlines=0, 
		acc=0,coeff_city=0,coeff_state=0,coeff_pin=0,coeff_chassis=0, coeff_reg=0, final_score=0,gendersim;
		...
		int linecount=0;
		BufferedReader br = new BufferedReader(new FileReader("/home/guddu/Lumiq/Ergo/Ergo_sample.tsv"));
		String line="";
		...
		{  
			if (linecount>0)
			{
			   ...
			   //Right tuple
			    ...
			    double addrsim = JaroSimilarity.findSim(addresses_left,addresses_right);
			    ...
			    HashMap<String,double[]> coefficientDict = ConfigParser.parseConfig();
			    //HashMap<String,Double> thresholdDict = ConfigParser.parseThresholdConfig();
			  
			    /************************************************************/
				//DOB and GENDER: Yash
				/***********************************************************/
				if(dobsim==1)
					coeff_dob = coefficientDict.get("dob")[1];
				else
					coeff_dob=coefficientDict.get("dob")[0];;
				if(gendersim==1)
					coeff_gender=coefficientDict.get("gender")[1];
				else
					coeff_gender=coefficientDict.get("gender")[1];
				/************************************************************/
				//NAME
				...
				if (namesim>=0.85)
					coeff_name=coefficientDict.get("fullname")[1];	//Same name, different people
				else
				{
					...
					if (gender_left.equals("FEMALE"))
					{
						if (namesim!=-0.5)
						{
							...
							if(firstname_left.equals(firstname_right))
							{
								...
								coeff_name=coefficientDict.get("fullname")[1];
							}
							...
							else
							{
								...
								coeff_name=coefficientDict.get("fullname")[0];
							}
						}
						else
							coeff_name=0;
				...
				{
					coeff_address2=coefficientDict.get("addresses")[1];
				}
				else
					coeff_address2=coefficientDict.get("addresses")[0];
				
				//City matching
				if (citysim>0.7)	//Same house
				{
					coeff_city=coefficientDict.get("city")[1];
				}
				else
					coeff_city=coefficientDict.get("city")[0];
				
				//State matching
				if (statesim>0.7)	//Same house
				{
					coeff_state=coefficientDict.get("state")[1];
				}
				else
					coeff_state=coefficientDict.get("state")[0];
				//Pin matching
				if (pinsim>0.7)	//Same house
				{
					coeff_pin=coefficientDict.get("pin")[1];
				}
				else
					coeff_pin=coefficientDict.get("pin")[0];
				/************************************************************/
				//Chassis number match
				/***********************************************************/
				if (chassissim==1)	//Anirban: Might change
					coeff_chassis=coefficientDict.get("chassisnum")[1];
				else
					coeff_chassis=coefficientDict.get("chassisnum")[0];	//Multiple chassis numbers can be there with one person
				/************************************************************/
				//Registration number match
				/***********************************************************/
				if (regsim==1)
					coeff_reg=coefficientDict.get("regnum")[1];
				else
					coeff_reg=coefficientDict.get("regnum")[0];
				...
				{
					coeff_contact=coefficientDict.get("contactnumbers")[1];	//Parents' phone number
					if(namesim>=0.7)	//Yash
					{
						namesim2=0;
						coeff_name=coefficientDict.get("fullname")[1];
					...
					coeff_contact=coefficientDict.get("contactnumbers")[0];	//Multiple contacts	
				...
				{
					coeff_id=coefficientDict.get("pan")[1];	//Strong influence if any ID attribute matches
					if(namesim>=0.7)	//Yash
					{
						namesim2=0;
						coeff_name=coefficientDict.get("fullname")[1];
				...
				//If no ID attributes are present
					...
					coeff_id=coefficientDict.get("pan")[0];
				...
				{
					coeff_email=coefficientDict.get("email")[1];	//Sometimes wife provides husband's email
					if(namesim>=0.7)	//Yash
					{
						namesim2=0;
						coeff_name=coefficientDict.get("fullname")[1];
					...
					coeff_email=coefficientDict.get("email")[0];
				...
				if((namesim>0.85)&&(dobsim==1)&&(gendersim==1)&&(addrsim<0.7)&&(idsim<1)&&(emailsim<1)&&(phsim<1))
				{
					if(final_score>=relaxed_PM_threshold)	//PAN, EIA, CKYC to be considered
					{	
